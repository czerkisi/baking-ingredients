...
public class Measure implements Ingredient {
    ...
    Measure(int numerator, int denominator, Ingredient ingredient){
        ...
        if ((double) numerator/denominator > 1){
            measuredIngredient = new SimpleIngredient(((double)numerator/denominator)
                    *ingredient.getCalories(), ((double)numerator/denominator), ingredient.isDry(),
                    numerator+"/"+denominator+" Cups "+ingredient.getName());

        } else {
            measuredIngredient = new SimpleIngredient(((double)numerator/denominator)
                    *ingredient.getCalories(), ((double)numerator/denominator), ingredient.isDry(),
                    numerator+"/"+denominator+" Cup "+ingredient.getName());
